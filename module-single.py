"""
This program calculates the 'shadow' of a single photovoltaik module.
...
mypic_shadow = "shadow-single.png"    # file name for result picture
...
offy = 5.0 # offset in x for placing the module
#### internal
...
sim_start = '2023-05-01 00:00'      
sim_end = '2023-05-31 23:00'
...
def make_modules(a):
    ...
    return Path(ps, codes)
#####################################################    
def beleucht(mx, my, mz, myshade, mdiff, mdir):
    # Belichte das Gitter -> wo ist Schatten, wo ist Licht 
    j = 0
    for y in my:
        for x in mx:
            mys = myshade.contains_points([(x,y)])
            if mys[0]:
                c = mdiff
            else:
                c = mdir + mdiff
...
while i < len(day):
    if day['Gd(i)'].iloc[i] > 0.0:
        ...
        # Erzeuge 2D Schatten        
        myshade = make_shape(px, py)
        
        ### belichte das Gitter
        #if h == 9 or h == 10:  # nur bestimmte Stunden ausgeben
        beleucht(mx, my, mz, myshade, day['Gd(i)'].iloc[i], float( day['Gb(i)'].iloc[i] ) )
